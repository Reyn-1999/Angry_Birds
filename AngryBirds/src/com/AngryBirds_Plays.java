...
public class AngryBirds_Plays extends JPanel{
	...
	public AngryBirds_Plays(boolean run,int level) throws IOException{
		/**/
		this.level = level;
		this.redBirdNum = 0;
		...
		this.backgroundImage = ImageIO.read(new File("picture/background2.png"));
		this.branch = new BufferedImage[2];
		this.branch[0] = ImageIO.read(new File("picture/branch1.png"));
		this.branch[1] = ImageIO.read(new File("picture/branch2.png"));
		this.width = 5.0f;
		this.centreX = 200;
		this.centreY = 630;
		this.score = 0;
	...
	*/
	...
	public void paintComponent(Graphics g){
		...
		if(bird.getX() > 200 && bird.isDragged() == false && bird.isChosen() == true){
			bird.setFly(true);
		}
	...
	private void drawPig(Graphics g){
		...
		boolean tmp = true;
		for(int i = 0; i < pigInitNum; ++i){
			// existed variable is changed in the pig's thread
			if(pig[i].isExisted()){
				tmp = false;
		...
		MouseListener ml = new MouseListener(){
			...
			public void mousePressed(MouseEvent e){
				...
				if(bird.isChosen(e.getX(), e.getY())){
					if(bird.isChosen()){
						bird.setDragged(true);	// this bird is dragged by the mouse
					}
				}
			}
	...
	private void setBirdChosen(AngryBirds_Bird chosenBird){
		for(int i = 0; i < this.redBirdNum; ++i)
		...
		chosenBird.setChosen(true);
		if(bird.isChosen(140, 700))
			bird.moveToInit();
		bird = chosenBird;
		bird.moveTo(140,700);
